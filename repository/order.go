...
func Add_Order(user_id, Address_id int) (int, error) {
	...
	result = database.DB.Raw("INSERT INTO orders (user_id,address_id,total_cart_amount,total_amount)VALUES(?,?,?,?) WHERE order_status is null;", user_id, Address_id, cart, cart).Scan(&models.Order{})
...
func OrderUpdation(payment_id string, user_id int) error {
	result := database.DB.Raw("UPDATE orders SET payment_method=$1,payment_status=$2,payment_id=$3,order_status=$4 WHERE user_id=$5;", "Razor_pay", "success", payment_id, "success", user_id).Scan(&models.Order{})
	if result.Error != nil {
		return result.Error
	}

	result = database.DB.Raw("UPDATE ordered_products SET payment='done' WHERE user_id=$1;", user_id).Scan(&models.OrderedProduct{})
	if result.Error != nil {
		return result.Error
	}
	return nil
