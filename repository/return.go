...
func ReturnProduct(user_id int, order_id, product_id string) error {
	...
	if order.Coupen == "applied" {
		...
		if coupen.MinAmount <= (order.TotalCartAmount - total_product_price) {
			...
			var money int
			database.DB.Raw("SELECT money FROM wallets WHERE user_id=$1;", user_id).Scan(&money)
			if money == 0 {
				database.DB.Raw("INSERT INTO wallets(user_id, money)VALUES($1,$2);", user_id, total_product_price).Scan(&models.Wallet{})
			} else {
				database.DB.Raw("UPDATE wallets SET money=$1 WHERE user_id=$2;", money+total_product_price, user_id).Scan(&models.Wallet{})
			}
			...
			if money == 0 {
				...
				database.DB.Raw("UPDATE wallets SET money=$1 WHERE user_id=$2;", money+total_product_price-coupen.Amount, user_id).Scan(&models.Wallet{})
			}
		}
	} else {
		price := order.TotalCartAmount - total_product_price
		...
		if money == 0 {
			...
			database.DB.Raw("INSERT INTO wallets(user_id, money)VALUES($1,$2);", user_id, money+total_product_price).Scan(&models.Wallet{})
		}
	}
	return nil
}
